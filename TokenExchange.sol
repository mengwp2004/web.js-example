...
contract GustoMainToken is CappedToken, BurnableToken, DetailedERC20 {
    ...
    event IssueToken(string name,string symbol,uint total,uint8 decimals,uint256 mainHold,address contractAddress);
    ...
    /*function issueToken(string name,string symbol,uint total,uint8 decimals,uint256 mainHold,address contractAddress) onlyOwner public returns(bool) {
        ...
        GustoChildTokenT sub = GustoChildTokenT(contractAddress);

        tokenRelations[symbol] = TokenRelation(symbol,mainHold,total,mainHold,total,sub,true);
        emit IssueToken(name,symbol,total,decimals,mainHold,sub);
        return true;
    }*/

    function issueToken(string name,string symbol,uint total,uint8 decimals,uint256 mainHold)  public returns(bool) {
        require(
            balances[msg.sender] >= mainHold,
            "balance must >= mainHold."
        );
        ...
        GustoChildTokenT sub = new GustoChildTokenT(name,symbol,total,decimals,this,owner,owner);
        transfer(owner,mainHold);
        tokenRelations[symbol] = TokenRelation(name,symbol,total,decimals,mainHold,total,mainHold,total,sub,true);
        emit IssueToken(name,symbol,total,decimals,mainHold,sub);
        return true;
    }
   ...
   function getDefaultRelation(string symbol)  public view  returns (uint256) {
    ...
    function getChildBalance(string symbol) public view returns (uint) {
        ...
        GustoChildTokenT sub = tokenRelation.childToken; 
...
contract GustoMainoken is MainToken {

    using SafeMath for uint256;
    MainToken mainToken;
    constructor(address contractAddress) public{
  	  mainToken = MainToken(contractAddress);
    }

}*/

contract GustoChildTokenT is  StandardToken,BurnableToken  {
    ...
    address public owner;
    GustoMainToken mainToken;
    
    event ChildRedeem(address from,uint256 value);
    event ChildPledge(address from,uint256 value);
    ...
    constructor(string n,string s,uint t,uint8 d,address mainContractAddress,address o,address m)  public {
        
        name = n;
        symbol = s;
        decimals = d;
        owner = o;
        uint256 total = t*10**(uint(decimals));
        balances[m] = total;
        totalSupply = totalSupply.add(total);
        mainToken = GustoMainToken(mainContractAddress);
        
    }
    
    function childRedeem(uint256 value) public returns(bool) {
        
        transfer(owner,value);
        TokenRecord storage tokenRecord = tokenRecords[msg.sender];
        uint256 _value = value.add(tokenRecord.value);
        tokenRecords[msg.sender] = TokenRecord(_value);
        emit ChildRedeem(msg.sender,_value);
        return true;
    }
    
    function getRedeemRecord(address _from) public view returns(uint256 value){
        TokenRecord storage tokenRecord = tokenRecords[_from];
        return  tokenRecord.value;
    }
    
    function childPledge() public returns (bool) {
        //check address and value
        //通过address获取对应的tokenRelation,然后减少对应的子币，加对应的主币
        ...
        uint256 value = mainToken.getPledgeRecord(symbol);
        require(value <= 0,"main token value equal 0!");
        TokenRecord storage tokenRecord = tokenRecords[_to];
       
        if(tokenRecord.value >= value){
            return false;
        }
        uint256 _value = value.sub(tokenRecord.value);
        address _from = owner;
        require(_value <= balances[_from]);
    
        balances[_from] = balances[_from].sub(_value);
        balances[_to] = balances[_to].add(_value);
        tokenRecords[_to] = TokenRecord(value);
        
        emit ChildPledge(_from,value);
        return true;
    }
